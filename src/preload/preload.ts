...
const bridge: PosBridge = {
  ...
  quotations: {
    list: (filters?: { status?: string; customer_id?: number; from_date?: string; to_date?: string; search?: string }) =>
      ipcRenderer.invoke('quotations:list', filters),
    get: (id: number) => ipcRenderer.invoke('quotations:get', id),
    create: (input: any) => ipcRenderer.invoke('quotations:create', input),
    updateStatus: (id: number, status: string) => ipcRenderer.invoke('quotations:updateStatus', id, status),
    delete: (id: number) => ipcRenderer.invoke('quotations:delete', id),
    convertToSale: (id: number, actorUserId: number) => ipcRenderer.invoke('quotations:convertToSale', id, actorUserId),
    expireOld: () => ipcRenderer.invoke('quotations:expireOld'),
  },
  templates: {
    list: (type?: string) => ipcRenderer.invoke('templates:list', type),
    get: (id: number) => ipcRenderer.invoke('templates:get', id),
    getDefault: (type: string) => ipcRenderer.invoke('templates:getDefault', type),
    create: (input: any) => ipcRenderer.invoke('templates:create', input),
    update: (id: number, input: any) => ipcRenderer.invoke('templates:update', id, input),
    delete: (id: number) => ipcRenderer.invoke('templates:delete', id),
    duplicate: (id: number, newName: string) => ipcRenderer.invoke('templates:duplicate', id, newName),
  },
  variants: {
    list: (productId: number) => ipcRenderer.invoke('variants:list', productId),
    get: (id: number) => ipcRenderer.invoke('variants:get', id),
    findByBarcode: (barcode: string) => ipcRenderer.invoke('variants:findByBarcode', barcode),
    create: (input: any) => ipcRenderer.invoke('variants:create', input),
    update: (id: number, input: any) => ipcRenderer.invoke('variants:update', id, input),
    delete: (id: number) => ipcRenderer.invoke('variants:delete', id),
    autoGenerate: (input: any) => ipcRenderer.invoke('variants:autoGenerate', input),
  },
  attributes: {
    list: () => ipcRenderer.invoke('attributes:list'),
    getValues: (attributeId: number) => ipcRenderer.invoke('attributes:getValues', attributeId),
    create: (name: string) => ipcRenderer.invoke('attributes:create', name),
    addValue: (attributeId: number, value: string) => ipcRenderer.invoke('attributes:addValue', attributeId, value),
  },
  credits: {
    check: (customerId: number, additionalAmount?: number) => ipcRenderer.invoke('credits:check', customerId, additionalAmount),
    setCustomerLimit: (customerId: number, limit: number, blockOnExceed: boolean, reason?: string, warningThresholdPct?: number) =>
      ipcRenderer.invoke('credits:setCustomerLimit', customerId, limit, blockOnExceed, reason, warningThresholdPct),
    setSupplierLimit: (supplierId: number, limit: number, blockOnExceed: boolean, warningThresholdPct?: number) =>
      ipcRenderer.invoke('credits:setSupplierLimit', supplierId, limit, blockOnExceed, warningThresholdPct),
    history: (customerId: number, limit?: number) => ipcRenderer.invoke('credits:history', customerId, limit),
    listRisks: () => ipcRenderer.invoke('credits:listRisks'),
  },
  branches: {
    list: () => ipcRenderer.invoke('branches:list'),
    get: (id: number) => ipcRenderer.invoke('branches:get', id),
    getDefault: () => ipcRenderer.invoke('branches:getDefault'),
    getCurrent: () => ipcRenderer.invoke('branches:getCurrent'),
    create: (input: any) => ipcRenderer.invoke('branches:create', input),
    update: (id: number, input: any) => ipcRenderer.invoke('branches:update', id, input),
    delete: (id: number) => ipcRenderer.invoke('branches:delete', id),
    setCurrent: (branchId: number) => ipcRenderer.invoke('branches:setCurrent', branchId),
  },
  transfers: {
    party: {
      list: () => ipcRenderer.invoke('transfers:party:list'),
      create: (input: any) => ipcRenderer.invoke('transfers:party:create', input),
    },
    bank: {
      listAccounts: () => ipcRenderer.invoke('transfers:bank:accounts'),
      createAccount: (input: any) => ipcRenderer.invoke('transfers:bank:createAccount', input),
      list: () => ipcRenderer.invoke('transfers:bank:list'),
      create: (input: any) => ipcRenderer.invoke('transfers:bank:create', input),
    },
  },
  fifo: {
    isEnabled: () => ipcRenderer.invoke('fifo:isEnabled'),
    isStrict: () => ipcRenderer.invoke('fifo:isStrict'),
    availableBatches: (productId: number) => ipcRenderer.invoke('fifo:availableBatches', productId),
    allocate: (productId: number, qty: number) => ipcRenderer.invoke('fifo:allocate', productId, qty),
    stockReport: (productId?: number) => ipcRenderer.invoke('fifo:stockReport', productId),
  },
  commissions: {
    rules: () => ipcRenderer.invoke('commissions:rules'),
    createRule: (input: any) => ipcRenderer.invoke('commissions:createRule', input),
    updateRule: (id: number, input: any) => ipcRenderer.invoke('commissions:updateRule', id, input),
    deleteRule: (id: number) => ipcRenderer.invoke('commissions:deleteRule', id),
    salesmen: () => ipcRenderer.invoke('commissions:salesmen'),
    calculate: (saleId: number) => ipcRenderer.invoke('commissions:calculate', saleId),
    list: (filters?: { salesman_id?: number; status?: string; from?: string; to?: string }) => ipcRenderer.invoke('commissions:list', filters),
    updateStatus: (id: number, status: string, userId: number) => ipcRenderer.invoke('commissions:updateStatus', id, status, userId),
    summary: (salesmanId: number, from?: string, to?: string) => ipcRenderer.invoke('commissions:summary', salesmanId, from, to),
  },
  expenses: {
    categories: () => ipcRenderer.invoke('expenses:categories'),
    createCategory: (input: any) => ipcRenderer.invoke('expenses:createCategory', input),
    updateCategory: (id: number, input: any) => ipcRenderer.invoke('expenses:updateCategory', id, input),
    deleteCategory: (id: number) => ipcRenderer.invoke('expenses:deleteCategory', id),
    list: (filters?: { category_id?: number; user_id?: number; from?: string; to?: string; status?: string }) => ipcRenderer.invoke('expenses:list', filters),
    ...
    summary: (from?: string, to?: string) => ipcRenderer.invoke('expenses:summary', from, to),
  },
  customReports: {
    list: (userId?: number) => ipcRenderer.invoke('reports:custom:list', userId),
    get: (id: number) => ipcRenderer.invoke('reports:custom:get', id),
    create: (input: any) => ipcRenderer.invoke('reports:custom:create', input),
    update: (id: number, input: any) => ipcRenderer.invoke('reports:custom:update', id, input),
    delete: (id: number) => ipcRenderer.invoke('reports:custom:delete', id),
    tables: () => ipcRenderer.invoke('reports:custom:tables'),
    schema: (table: string) => ipcRenderer.invoke('reports:custom:schema', table),
    execute: (id: number, limit?: number) => ipcRenderer.invoke('reports:custom:execute', id, limit),
    schedules: () => ipcRenderer.invoke('reports:custom:schedules'),
    createSchedule: (input: any) => ipcRenderer.invoke('reports:custom:createSchedule', input),
    updateSchedule: (id: number, input: any) => ipcRenderer.invoke('reports:custom:updateSchedule', id, input),
    deleteSchedule: (id: number) => ipcRenderer.invoke('reports:custom:deleteSchedule', id),
  },
};
